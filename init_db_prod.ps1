# init_db_prod.ps1 - Инициализация базы данных FitMan для PROD окружения (Windows PowerShell)
# Использование: .\init_db_prod.ps1

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Инициализация PROD базы данных FitMan" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# Загрузка переменных окружения из backend\.env.prod
Write-Host "Загрузка настроек из backend\.env.prod..." -ForegroundColor Yellow
if (Test-Path "backend\.env.prod") {
    Get-Content "backend\.env.prod" | ForEach-Object {
        $line = $_.Trim()
    ...
    Write-Host "✓ Настройки для prod окружения загружены." -ForegroundColor Green
} else {
    Write-Host "✗ Ошибка: Файл окружения 'backend\.env.prod' не найден!" -ForegroundColor Red
...
$DB_PASSWORD = if ($env:DB_PASS) { $env:DB_PASS } else { "postgres" }
